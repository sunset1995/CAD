...
double output_result(char* fpos, bool is_output)
{
	int i, j;
	double CA, CB;
	FILE *fout;
	...
	for (i = 0; i<(int)Win.size(); i++){
		if (is_output == false) {
			...
			CA = (double)bestColor.CA[i] * 100 / ((double)omega*(double)omega);
			CB = (double)bestColor.CB[i] * 100 / ((double)omega*(double)omega);
		}
		if (is_output) {
